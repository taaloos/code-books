pn={
  ...
  init:function(){
    ...
    for(var i=0;i<ts.length;i++){
      ...
      for(j=0;j<ts[i].datarows.length;j++){
	    ts[i].datarows[j].style.display='none';
	  }
  ...
  createPaginationNav:function(table){
	...
	table.bottomCounter=navBefore.getElementsByTagName('span')[0];
  },
  navigate:function(e){
    var start,table;
  ...
  showSection:function(table,start){
    ...
    if(table.current != null){
      for(i=table.current;i<table.current+pn.increase;i++){
        if(table.datarows[i]){
          table.datarows[i].style.display='none';
        }
      }
    }
    for(i=start;i<start+pn.increase;i++){
      if(table.datarows[i]){
       table.datarows[i].style.display='';
      }
    }
    table.current=start;
  },
  changePaginationNav:function(table,start){
    ...
    if(start+pn.increase>table.rowsize-1){
