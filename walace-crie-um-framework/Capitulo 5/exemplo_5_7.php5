...
class teste extends BASE {
	public function __construct(BancoDados $_conn) {
		...
		$this->addCampo(new string("AJAX","Unidade Federativa",30,null,null,true,true,true,null,null,null,false));
		$this->getCampo("AJAX")->setComportamento_form("ajax");
		$this->getCampo("AJAX")->Seturl_ajax("retorna_ufs.php5");
	}
}
$_t = new teste($_bd);
...
$_html->getLastSubTag()->addSubTag(new tag(new tipotag("SCRIPT"),Array(new atributo("SRC","framework/javascript/scriptaculous/scriptaculous.js"))));
