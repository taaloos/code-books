...
01.12.2006 - Walace Soares - Improvements in Calendar Desing and Date processing

*****************************************************************************/
//constructor for the main Epoch class (PORTUGUESE VERSION)
function Epoch(name,mode,targetelement,multiselect,ativar)
{
	try {
	...
	if(this.mode == 'popup' && targetelement && targetelement.type == 'text') //if the target element has been set to be an input text box
	{
		...
		this.calendar.style.top = this.getTop(targetelement) + this.topOffset +  2 + 'px';
		var px = this.getLeft(targetelement) + this.leftOffset;
		//alert(px + ' : ' + this.calendar.width + ' :: ' + document.body.scrollWidth);
		if((px+280)>document.body.scrollWidth) {
			px = document.body.scrollWidth - 280 - 10;
		};
		this.calendar.style.left = px + 'px';
		document.body.appendChild(this.calendar);
		this.tgt.calendar = this;
		if(this.tgt.value!='') {
			// tentar converter a data..
			var d = this.newDate(this.tgt.value);
			if(d!==false) {
				this.addDates(new Array(d),true);
			}	
		}
	...
	this.visible | ativar===true ? this.show() : this.hide();
};
// Ano Bissexto
Epoch.prototype.ano_bi = function(ano) 
{
	if (ano % 100 == 0) {
		if (ano % 400 == 0) 
			return true; 
...
// Gera uma data
Epoch.prototype.newDate = function(data) 
{
...
{
	this.daylist = new Array('Dom','Seg','Ter','Qua','Qui','Sex','Sáb','Dom','Seg','Ter','Qua','Qui','Sex','Sáb');
	this.months_sh = new Array('Jan','Fev','Mar','Abr','Mai','Jun','Jul','Ago','Set','Out','Nov','Dez');
	this.monthup_title = 'Próximo mês';
	this.monthdn_title = 'Mês anterior';
	this.clearbtn_caption = 'Limpar';   
	this.close_caption  = 'Fechar';
	this.clearbtn_title = 'Limpa todas as datas seleccionadas no calendário';
	this.maxrange_caption = 'Este é o limite máximo';
	this.close_title = 'Fecha o Calendário';
};
//-----------------------------------------------------------------------------
...
//	tbody.appendChild(tr);
	...
	//this.calendar.onmouseout = function() { if(this.owner.mode == 'popup' & this.owner.mousein === true) { this.owner.hide(); } };
};
//-----------------------------------------------------------------------------
Epoch.prototype.createMainHeading = function () //PRIVATE: Creates the primary calendar heading, with months & years
{
	...
	// outros controles																	
	if(this.mode == 'popup') {
		var hoje = document.createElement('input'), sair = document.createElement('input');
		hoje.setAttribute('type','button');
		hoje.setAttribute('value','Hj');
		hoje.setAttribute('title','Ir para a Data de Hoje');
		sair.setAttribute('type','button');
		sair.setAttribute('value',' X ');
		sair.setAttribute('title',' Sair ');
		hoje.owner = this;
		sair.owner = this;
		hoje.onclick = function() { 
							this.owner.goToMonth(this.owner.displayYearInitial,this.owner.displayMonthInitial);
						 };
		sair.onclick = function() {if(this.owner.mode == 'popup') { this.owner.hide(); this.owner.reDraw(); }};
	};
	
	//and finally add the elements to the containing div
	if(this.mode == 'popup') {
		container.appendChild(hoje);
	};	
...
Epoch.prototype.createFooter = function () //PRIVATE: creates the footer of the calendar - goes under the calendar cells
{
	...
	closeSelected.setAttribute('value',this.close_caption);
	closeSelected.setAttribute('title',this.close_title);
	closeSelected.owner = this;
	closeSelected.onclick = function() { 
								if(this.owner.mode == 'popup') { 
...
//-----------------------------------------------------------------------------
Epoch.prototype.createDayHeading = function (tbody)  //PRIVATE: creates the heading containing the day names
{
	...
	{
		td = document.createElement('td');
		td.setAttribute('class','nothing');
		td.setAttribute('className','nothing'); //<iehack>
		tr.appendChild(td);
	};
	...
	{
		td = document.createElement('td');	  
		td.setAttribute('class','hday');
		td.setAttribute('className','hday'); //<iehack>
...
Epoch.prototype.createCalCells = function ()  //PRIVATE: creates the table containing the calendar day cells
{
	...
	// Aqui devemos incluir o cabeçalho, só assim teremos o mesmo tamanho...
	this.createDayHeading(tbody);
	...
	if(!owner.selCurMonthOnly || cell.date.getMonth() == owner.displayMonth && cell.date.getFullYear() == owner.displayYear)
	{
		...
		else //if we can only select one cell at a time
		{
			...
			if(owner.tgt) //if there is a target element to place the value in, do so
			{
				...
				try{var f=new FADE();f.PadraoCor();f.setElementos([owner.tgt.id]);f.Run();}catch(e){};
				if(owner.mode == 'popup') {
					owner.hide();
				}
			}
...
Date.prototype.dateFormat = function(format)
{
	...
	var DAY_NAMES = new Array('Domingo','Segunda','Terça','Quarta','Quinta','Sexta','Sábado','Dom','Seg','Ter','Qua','Qui','Sex','Sáb');
	format = format + "";
	var result="";
	var i_format=0;
	var c="";
