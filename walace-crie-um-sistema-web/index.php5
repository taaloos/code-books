...
$_main->getLastSubTag()->addSubTag(new tag(new tipotag("LINK",false),
									Array(new atributo("REL","STYLESHEET"),
										  new atributo("TYPE","text/css"),
										  new atributo("HREF","framework/javascript/dialog/dialog_box.css"))));
...
// NOVO
$_main->getLastSubTag()->addSubTag(new tag(new tipotag("SCRIPT"),Array(new atributo("SRC","framework/javascript/dialog/dialog_box.js"))));
...
if($_POST['ACAO']=='SALVAR') {
	...
	if(($_pos=strpos($_file['dirname'],"/",1))!==false) {
		$_dirextra =  substr($_file['dirname'],$_pos+1);
		chdir($_dirextra);
	}
	...
	if($_COOKIE['ULTIMO_PROGRAMA']!==null&&$_COOKIE['ULTIMO_PROGRAMA']!="index.php5") {
		$_main->getLastSubTag()->getLastSubTag()->AddSubTag(new tag($_tipos->getTipo('SCRIPT'),null,
