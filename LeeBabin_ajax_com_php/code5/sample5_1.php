				...
				for ($i = 0; $i < 7; $i++){
					...
					if (($startdate <= 0) || ($startdate > $lastday)){
						...
						if ($startdate == date("j") && $month == date("n") && $year == date("Y")){
							?><td onclick="createform(event,'<?php echo $year . "-" . $month . "-" . $startdate; ?>')" style="text-align: center; background: #FFBC37;" onmouseover="this.style.background='#FECE6E'; checkfortasks ('<?php echo $year . "-" . $month . "-" . $startdate; ?>',event);" onmouseout="this.style.background='#FFBC37'; hidetask();"><?php echo date ("j"); ?></td><?php
						} else {
							?><td onclick="createform(event,'<?php echo $year . "-" . $month . "-" . $startdate; ?>')" style="text-align: center; background: #A2BAFA;" onmouseover="this.style.background='#CAD7F9'; checkfortasks ('<?php echo $year . "-" . $month . "-" . $startdate; ?>',event);" onmouseout="this.style.background='#A2BAFA'; hidetask();"><?php echo $startdate; ?></td><?php
						}
					}
				}
				?></tr><?php
